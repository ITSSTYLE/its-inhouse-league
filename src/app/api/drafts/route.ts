...
const { data, error } = await supabase
  .from("draft_lobbies")
  .select(
    "*, blue_captain:players!draft_lobbies_blue_captain_id_fkey(*), red_captain:players!draft_lobbies_red_captain_id_fkey(*)"
  )
  .order("created_at", { ascending: false });

return NextResponse.json({
  drafts: assertSupabase(data, error)
});
```
...
if (body.blueCaptainId === body.redCaptainId) {
  return NextResponse.json(
    { error: "Captains must be different players" },
    { status: 400 }
  );
}
...
return NextResponse.json(
  {
    draft: assertSupabase(data, error)
  },
  { status: 201 }
);
```
