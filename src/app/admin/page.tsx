...
export default async function AdminPage() {
  ...
  const [{ data: players }, { data: drafts }, { data: matches }] = await Promise.all([
    supabase.from("players").select("*").order("username", { ascending: true }),
    supabase.from("draft_lobbies").select("*").order("created_at", { ascending: false }).limit(5),
    ...
    <div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm lg:col-span-2">
          <h2 className="text-lg font-black text-slate-950">Create Draft Lobby</h2>
          <AdminCreateDraftForm players={players ?? []} />
        </section>
        <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          <h2 className="text-lg font-black text-slate-950">League Health</h2>
